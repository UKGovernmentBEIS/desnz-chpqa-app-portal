...
namespace WebApi.Controllers
{
    [Route("api")]
    [ApiController]
    public class GetSubmissionByid : ControllerBase
    {
        private readonly ILogger<GetSubmissionByid> _logger;
        private readonly IServiceClientFactory _serviceClientFactory;
        public GetSubmissionByid(ILogger<GetSubmissionByid> logger, IServiceClientFactory serviceClientFactory)
        {
            _logger = logger;
            _serviceClientFactory = serviceClientFactory;
        }
        /// <summary>
        /// Retrieves Details of Submission By Submission id
        /// </summary>
        /// <remarks>
        /// 
        /// </remarks>
        ...
        [SwaggerOperation(Summary = "Get Submission Details By Submission id", Description = "Retrieves Details of Submission By Submission id")]
        public async Task<ActionResult<ReplySubmission>> GetSubmissionDetailsByID([Required]Guid Id)
        {
            try
            {
                using (var serviceClient = _serviceClientFactory.CreateServiceClient())
                {
                    ...
                    QueryExpression query = new QueryExpression
                    {
                        ...
                        Criteria = new FilterExpression
                        {
                            Conditions =
                            {
                                new ConditionExpression("desnz_submissionid", ConditionOperator.Equal, Id)
                            }
                        },
                        LinkEntities =
                        {
                    ...
                    if (results.Entities.Count > 0)
                    {
                        ...
                        //
                        if (!await AuthorizationService.ValidateUserData(User, serviceClient, schemeId)) return StatusCode(StatusCodes.Status401Unauthorized);
                        //
