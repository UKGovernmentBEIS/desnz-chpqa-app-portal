...
namespace WebApi.Middleware
{
    ...
    public class GlobalFunctionMiddleware
    {
        private readonly RequestDelegate _next;
        private readonly IServiceClientFactory _serviceClientFactory;

        public GlobalFunctionMiddleware(RequestDelegate next, IServiceClientFactory serviceClientFactory)
        {
            _next = next;
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            // If value is null or invalid, avoid processing claims if unnecessary
            if (value == null)
            {
                context.Response.StatusCode = StatusCodes.Status401Unauthorized;
                await context.Response.WriteAsync("Unauthorized");
                return; // Early exit, don't call next middleware
            }
        ...
        private async Task<bool> YourGlobalFunction(string value, string userClaim)
        {
            ...
            {
                return await AuthorizationService.ValidateUserDataSubid(userClaim, serviceClient, new Guid(value));
            }
        }
