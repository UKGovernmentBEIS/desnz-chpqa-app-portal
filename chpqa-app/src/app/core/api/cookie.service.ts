...
export class CookieService {
  PREFERENCES_SET_COOKIE = 'chpqa_cookies_preferences_set';
  COOKIES_POLICY = 'chpqa_cookies_policy';

  /**
   * Accept all cookies in the application.
   */
  acceptAllCookies(cookiesExpirationTime: number = 365) {
    const d = new Date();
    d.setTime(
  ...
  acceptEssentialCookies(cookiesExpirationTime: number = 365) {
    const d = new Date();
    d.setTime(
      d.getTime() + Number(cookiesExpirationTime) * 24 * 60 * 60 * 1000
    ); // Valid for 1 year
  ...
  isPreferencesCookieSet(): boolean {
    console.log('this.getCookie(this.PREFERENCES_SET_COOKIE)',this.getCookie(this.PREFERENCES_SET_COOKIE))
    return this.getCookie(this.PREFERENCES_SET_COOKIE) === 'true';
  }
}
