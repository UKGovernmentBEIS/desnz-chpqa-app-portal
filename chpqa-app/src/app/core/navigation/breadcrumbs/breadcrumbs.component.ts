...
import { DestroySubjectService } from '../../services/destroy-subject.service';
...
//TODOO check and test BreadcrumbsComponent functionality after i have right url paths
@Component({
        ...
        <div *ngIf="breadcrumbs$ | async as breadcrumbs">
          <ng-container *ngFor="let breadcrumb of breadcrumbs; index as i">
            ...
            <ng-template #bareText>
              <li class="govuk-breadcrumbs__list-item" govukLink="breadcrumb">
                {{ breadcrumb.text }}
              </li>
            </ng-template>
          </ng-container>
...
export class BreadcrumbsComponent {
  constructor(
    ...
    private readonly destroy$: DestroySubjectService,
    @Inject(BREADCRUMB_ITEMS)
    protected breadcrumbs$: BehaviorSubject<BreadcrumbItem[]>
  ) {
    router.events
      .pipe(
  ...
  private getBreadcrumbLink(
    ...
    if (
      ...
      while (currentRoute.firstChild) {
        ...
        if (this.hasComponent(currentRoute)) {
          currentRouteUrl = currentRouteUrl.concat(
            currentRoute.url.map(url => url.path)
          );
          break;
        }
    ...
    return this.hasMoreUrlSegments(route)
      ? ['/' + currentRouteUrl.join('/')]
      : null;
  }

  private alreadyHasBreadcrumb(
    breadcrumbs: BreadcrumbItem[],
  ...
  private hasComponent(route: ActivatedRouteSnapshot): boolean {
    ...
    const childWithEmptyPath: Route =
      hasChildren && children.find(child => child.path.length === 0);

    return (
      (!!route.routeConfig.component && !hasChildren) ||
      !!childWithEmptyPath?.component
    );
  }
  ...
  private hasTextResolutionFunction(breadcrumb: any): boolean {
    return (
      ...
      'resolveText' in breadcrumb &&
      typeof breadcrumb.resolveText === 'function'
    );
  }
  ...
  private mustSkipLink(data: Data): boolean {
    return (
      ...
      ((typeof data['breadcrumb'].skipLink === 'boolean' &&
        data['breadcrumb'].skipLink === true) ||
        (typeof data['breadcrumb'].skipLink === 'function' &&
          data['breadcrumb'].skipLink(data) === true))
    );
  }
}
