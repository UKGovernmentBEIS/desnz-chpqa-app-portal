...
import { f2FormFeature, setDailyHeatProfile } from '../../../store';

@Component({
  selector: 'app-upload-daily-heat-profile',
  standalone: true,
  templateUrl: './upload-daily-heat-profile.component.html',
  styleUrl: './upload-daily-heat-profile.component.scss',
  imports: [
    CommonModule,
    ReactiveFormsModule,
    RouterModule,
...
export class UploadDailyHeatProfileComponent implements OnInit {
  form: FormGroup;
  files: FileWithId[] = [];
  ...
  ngOnInit(): void {
    this.store
      .select(f2FormFeature.selectDailyHeatProfile)
      .pipe(first())
      .subscribe(dailyHeatProfile => {
        this.files = dailyHeatProfile.files ? [...dailyHeatProfile.files] : [];
        this.form.patchValue({
          files: dailyHeatProfile.files.map((fileWithId: FileWithId) => fileWithId.file),
          comments: dailyHeatProfile.comments,
        });
      });
  }
  ...
  onContinue() {
    if (this.form.valid) {
      ...
      this.router.navigate([`../${FormSubmissionPath.DAILY_HEAT_PROFILE_SUMMARY}`], {
