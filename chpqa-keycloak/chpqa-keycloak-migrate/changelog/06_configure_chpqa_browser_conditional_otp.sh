...
#This script updates conditional OTP execution of CHPQA_Browser authentication flow.
...
#Get CHPQA Browser Conditional OTP execution id
GET_CHPQA_BROWSER_EXECUTIONS=$(curl -s -L -X GET "$UPDATE_REALM_URL$CHPQA_REALM_NAME"/authentication/flows/"$CHPQA_BROWSER"/executions \
-H 'Content-Type: application/json' \
-H "Authorization: Bearer $KEYCLOAK_ADMIN_ACCESS_TOKEN")

CONDITIONAL_OTP_ID=$(echo "$GET_CHPQA_BROWSER_EXECUTIONS" | jq -r '.[] | select(.displayName=="CHPQA_Browser Browser - Conditional OTP").id')

#Update Conditional OTP execution
CONDITIONAL_OTP_EXECUTION=$(curl -s -L -X PUT "$UPDATE_REALM_URL$CHPQA_REALM_NAME"/authentication/flows/"$CHPQA_BROWSER"/executions \
-H 'Content-Type: application/json' \
-H "Authorization: Bearer $KEYCLOAK_ADMIN_ACCESS_TOKEN" \
--data-raw '{
...
then
	...
	echo " Realm $CHPQA_REALM_NAME updated failed"
	exit;
else
